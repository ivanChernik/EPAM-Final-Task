...
public final class ConnectionPool {
	...
	private ConnectionPool() {
		...
		try {
			poolSize = Integer.parseInt(dbManager.getValue(DBParameter.DB_POOL_SIZE));
	...
	public void initConnectionPool() throws ConnectionPoolException {
		...
		try {
			...
			for (int indexPool = 0; indexPool < poolSize; indexPool++) {
				Connection originalConnection = DriverManager.getConnection(url, userName, password);
			...
			throw new ConnectionPoolException("Connection is not got", e);
		}
	...
	private void clearConnectionQueue() throws ConnectionPoolException {
		try {
			...
			throw new ConnectionPoolException("Error closing connection queues", e);
		}
	}
